...
for col in cols:
    print(col)
    df1 = df[[col, 'text']]
    X = np.reshape(df1['text'].values, (-1,1))
    y = np.reshape(df1[col].values, (-1,1))
    ...
    df1.to_csv(os.path.join('data', col+'.txt'), sep='\t', index=False, header=None)
...
df['text'] = df['text'].apply(preprocess)
df['label'] = 0
df[['label', 'text']].to_csv(os.path.join('data','newtest.txt'), sep='\t', index=False, header=None)
